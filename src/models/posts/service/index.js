...
export class PostService {
  ...
  async updatePost(postId, props, user) {
    ...
    if (post.userId !== user.id) throw { status: 403, message: "본인글만 수정이 가능합니다."}
  ...
  async updateComment(commentId, user) {
    ...
    if (comment.userId !== user.id) throw { status: 403, message: "댓글 수정 권한이 없습니다."}
  ...
  async deletePost(postId, user) {
    ...
    if (post.userId !== user.id) throw { status: 403, message: "삭제 권한이 없습니다."}
  ...
  async deleteComment(commentId, user) {
    ...
    if (comment.userId !== user.id) throw { status: 403, message: "삭제 권한이 없습니다."}
